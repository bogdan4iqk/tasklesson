...
export const renderAllUsers =  (users, renderTo) => {
	...
	users.forEach(user => {
		i++
		renderTo.insertAdjacentHTML(`beforeend`, `<li><div class="li${i}">${user.name}</div> <input type="text" class="input${i} input" placeholder="${user.name}"> <div class="edit${i} editBtn">V</div><div class="dlt${i} dltBtn" style="z-index: 100;">X</div></li>`)
